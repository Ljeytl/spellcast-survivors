...
func setup_audio_buses():
	...
	print("DEBUG: SFX bus at index: ", sfx_idx, " volume: ", AudioServer.get_bus_volume_db(sfx_idx))
	...
	print("DEBUG: Music bus at index: ", music_idx, " volume: ", AudioServer.get_bus_volume_db(music_idx))
...
func play_music(music_type: SoundType, loop: bool = true, fade_in_duration: float = 0.0):
	...
	print("DEBUG: play_music called with type: ", SoundType.keys()[music_type])
...
func load_audio_file(file_path: String) -> AudioStream:
	...
	if ResourceLoader.exists(file_path):
		print("DEBUG: Audio file exists, loading...")
		var stream = load(file_path)
		if stream:
			print("DEBUG: Successfully loaded audio stream of type: ", stream.get_class())
			return stream
		else:
...
func _on_music_finished():
	...
	print("DEBUG: Music finished playing - checking if it should loop")
	print("DEBUG: Music player valid: ", is_instance_valid(current_music_player))
	print("DEBUG: Music player in tree: ", current_music_player.is_inside_tree() if current_music_player else "null")
	print("DEBUG: Music player stream: ", current_music_player.stream.get_class() if current_music_player and current_music_player.stream else "null")
	...
	if current_music_player and current_music_player.stream:
		print("DEBUG: Manually restarting music...")
		current_music_player.play()
		is_music_playing = true
	else:
		print("DEBUG: Cannot restart - music player or stream is null")
...
func test_music_player_with_sfx():
	"""Test if the music player can play a simple SFX file"""
	print("DEBUG: Testing music player with SFX file...")
	
	var test_file = "res://audio/sfx/ui_button_click_1.wav"
	if ResourceLoader.exists(test_file):
		var stream = load(test_file)
		if stream:
			current_music_player.stream = stream
			current_music_player.volume_db = 0
			print("DEBUG: Playing SFX through music player...")
			current_music_player.play()
			...
			print("DEBUG: Music player SFX test completed")
